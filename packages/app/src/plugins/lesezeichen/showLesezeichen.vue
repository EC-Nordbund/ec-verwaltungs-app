    ...
    <template slot="activator">
      <v-badge overlap bottom color="accent" v-if="count > 0">
        ...
        <v-icon medium v-white>{{
          $liste.liste.length === 0 ? "star_border" : "star"
        }}</v-icon>
      </v-badge>
      <v-icon medium v-white v-else>star_border</v-icon>
    </template>
    <v-card>
      <v-card-title> <h1 v-font v-primary>Lesezeichen</h1> </v-card-title>
          ...
          <v-list-tile inactive>
            <v-list-tile-content>
              ...
              <v-list-tile-sub-title
                >Klicke auf das <v-icon small>star_border</v-icon>-Symbol auf
                den Details-Seiten um die jeweilige hinzuzufügen.
              </v-list-tile-sub-title>
            </v-list-tile-content>
          </v-list-tile>
        </v-list>
...
export default class showLesezeichen extends Vue {
  ...
  selectAll() {
    if (this.selectedBookmarks.length === this.$liste.liste.length) {
      this.selectedBookmarks = [100];
    } else {
      this.selectedBookmarks = Array.from(this.$liste.liste.keys());
    }
  }
