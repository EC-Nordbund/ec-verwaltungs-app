...
export class LesezeichenList extends EventEmitter {
  ...
  public contains(lesezeichen: Lesezeichen) {
    const el = this.liste.filter(
      v => v.elID === lesezeichen.elID && v.type === lesezeichen.type
    );
    return el.length > 0;
  }

  private save() {
    require("electron-settings").set("lesezeichen", JSON.stringify(this.liste));
    this.emit("changed");
    this.emit("changed2");
  }
  ...
  private load() {
    this.liste = <Array<Lesezeichen>>(
      JSON.parse(<string>require("electron-settings").get("lesezeichen", "[]"))
    );
  }
