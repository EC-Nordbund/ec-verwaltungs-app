<template>
  <gql-query :variables="{ force }">
    <template slot="query">
      query($authToken: String!, $force: Boolean!){ getDSE(authToken:
      $authToken, force: $force) }
    </template>
        ...
        <v-card>
          ...
          <v-card-actions>
            ...
            >
              ...
              <template slot="query">
                mutation($authToken: String!) { acceptsDSE(authToken:
                $authToken) }
              </template>
              <template slot="loading" /> <template slot="error" />
            </gql-mutate>
            <v-btn v-if="force" @click="isShow = false" v-primary-bg
              >Schließen</v-btn
            >
          </v-card-actions>
        </v-card>
...
@Component({})
export default class ecDsgvo extends Vue {
  show = false;
