  ...
  <div>
    <template>
      <template v-for="(item, index) in splitNumbers">
        <template v-if="page - 1 == index">
          <ec-single-page-liste
            v-if="splitNumbers.length == 1"
            :standard="standard"
            :items="items.slice(0, item)"
            :key="index"
            :extra="extraAtLast"
          />
          <ec-single-page-liste
          ...
          />
          <ec-single-page-liste
            v-else-if="index == splitNumbers.length - 1"
            :standard="standard"
            :items="items.slice(splitNumbers[index - 1], item)"
            :key="index"
            :extra="extraAtLast"
          />
          <ec-single-page-liste
    ...
    <template v-if="splitNumbers.length > 1">
      ...
      <v-btn
        icon
        @click="page = page + 1"
        :disabled="page === splitNumbers.length"
      >
