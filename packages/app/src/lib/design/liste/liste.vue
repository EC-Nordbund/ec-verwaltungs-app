    ...
    <template>
      <template v-for="(item, index) in splitNumbers">
        <template v-if="page-1==index">
          <ec-single-page-liste v-if="splitNumbers.length==1" :standard="standard" :items="items.slice(0,item)" :key="index" :extra="extraAtLast"/>
          <ec-single-page-liste v-else-if="index==0" :standard="standard" :items="items.slice(0,item)" :key="index"/>
          <ec-single-page-liste v-else-if="index==splitNumbers.length-1" :standard="standard" :items="items.slice(splitNumbers[index-1],item)" :key="index" :extra="extraAtLast"/>
          <ec-single-page-liste v-else :standard="standard" :items="items.slice(splitNumbers[index-1],item)" :key="index"/>
        </template>
      </template>
    </template>
...
@Component({})
export default class ecListe extends Vue {
  @Prop({ type: Array })
  items!: any
