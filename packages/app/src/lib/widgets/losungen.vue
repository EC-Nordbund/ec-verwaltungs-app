<template>
  <v-card class="elevation-10" style="margin: 5px">
    <v-card-title @click="openL" style="cursor: pointer;">
      <h1 v-font v-primary :style="{ textAlign: 'center' }">Die Losungen</h1>
      <v-spacer />
      <img width="64px" src="@/../public/losung.png" />
    </v-card-title>
    <template>
      <v-card-text
        v-if="losung.length > 0"
        :style="{
          display: 'grid',
      ...
      >
        ...
        <div v-font :style="{ fontSize: '12px' }">
          <a @click="openH"
            >© Evangelische Brüder-Unität – Herrnhuter Brüdergemeine</a
          >
        </div>
      </v-card-text>
      <v-card-text
      ...
      >
        ...
        <div v-font v-primary :style="{ fontSize: '22px' }">Loading...</div>
      </v-card-text>
    </template>
  </v-card>
</template>
...
@Component({})
export default class ecWidgetLosungen extends Vue {
  losung: Array<string> = [];

  async created() {
    const res = await window.fetch("https://www.losungen.de/heute.php");
