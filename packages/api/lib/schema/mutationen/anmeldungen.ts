...
export default {
  anmeldungBesonderheiten: {
    ...
    resolve: handleAllowed((_, args) => {
      return query(
        `UPDATE anmeldungen SET vegetarisch = ${
          args.vegetarisch
        }, lebensmittelAllergien="${
          args.lebensmittelAllergien
        }", gesundheitsinformationen="${
          args.gesundheitsinformationen
        }", bemerkungen="${args.bemerkungen}" WHERE anmeldeID="${
          args.anmeldeID
        }"`
      );
    }, "anmeldungBesonderheiten")
  ...
  anmeldungBezahlt: {
    ...
    resolve: handleAllowed((_, args) => {
      return query(
        `UPDATE anmeldungen SET bisherBezahlt = ${
          args.betrag
  ...
  anmeldungRueckbezahlt: {
    ...
    resolve: handleAllowed((_, args) => {
      query(
        `UPDATE anmeldungen SET rueckbezahlt = ${
          args.betrag
        } WHERE anmeldeID="${args.anmeldeID}"`
      );
    }, "anmeldungFinanzen")
  ...
  anmeldungKontakt: {
    ...
    resolve: handleAllowed((_, args) => {
      query(
        `UPDATE anmeldungen SET adressID=${args.adressID}, eMailID=${
          args.emailID
        }, telefonID=${args.telefonID} WHERE anmeldeID = '${args.anmeldeID}'`
      );
    }, "anmeldungKontakt")
  },
  abmelden: {
    ...
    resolve: handleAllowed((_, args) => {
      query(
        `UPDATE anmeldungen SET abmeldeZeitpunkt=CURRENT_TIMESTAMP,abmeldeGebuehr=${
          args.gebuehr
        },wegDerAbmeldung="${args.weg}", kommentarAbmeldung="${
          args.kommentar
        }" WHERE anmeldeID = "${args.anmeldeID}"`
      );
    }, "anmeldungAbmelden")
  ...
  nachruecken: {
    ...
    resolve: handleAllowed((_, args) => {
      query(
        `SELECT wartelistenPlatz, veranstaltungsID, geschlecht FROM anmeldungen, personen WHERE personen.personID = anmeldungen.personID AND anmeldeID = "${
          args.anmeldeID
        }"`
      )
        .then(row => row[0])
        .then(r => {
          query(
            `SELECT hatGWarteliste FROM veranstaltungen WHERE veranstaltungsID=${
              r.veranstaltungsID
            }`
          )
            .then(row => row[0])
            .then(v => {
              if (v.hatGWarteliste) {
                query(
                  `UPDATE anmeldungen SET wartelistenPlatz=0 WHERE anmeldeID="${
                    args.anmeldeID
                  }"`
                ).then(v => {
                  query(
                    `UPDATE anmeldungen SET wartelistenPlatz=wartelistenPlatz-1 WHERE wartelistenPlatz > ${
                      r.wartelistenPlatz
                    } AND personen.personID = anmeldungen.personID AND personen.geschlecht = "${
                      r.geschlecht
                    }"`
                  );
                });
  ...
  anmelden: {
    ...
    async resolve(_, args) {
      ...
      if (allowed) {
        ...
        writeFileSync(
          __dirname + "/../../../log/anmel_" + h + "_" + d + ".log.json",
          j
        );
        ...
        let persons = await query(
          `SELECT personID FROM personen WHERE vorname="${
            args.vorname
          }"AND  nachname="${args.nachname}" AND gebDat="${args.gebDat}"`
        );
        if (persons.length === 0) {
          await query(
            `INSERT INTO personen (vorname, nachname, gebDat, geschlecht) VALUES ("${
              args.vorname
            }", "${args.nachname}", "${args.gebDat}", "${args.geschlecht}")`
          );
          persons = await query(
            `SELECT personID FROM personen WHERE vorname="${
              args.vorname
        ...
        let eMails = await query(
          `SELECT eMailID FROM eMails WHERE eMail="${
            args.eMail
          }" AND personID=${personID}`
        );
        if (eMails.length === 0) {
          await query(
            `INSERT INTO eMails(eMail, personID) VALUES ("${
              args.eMail
            }",${personID})`
          );
          eMails = await query(
        ...
        let telefone = await query(
          `SELECT telefonID FROM telefone WHERE telefon="${
            args.telefon
          }" AND personID=${personID}`
        );
        ...
        let adressen = await query(
          `SELECT adressID FROM adressen WHERE personID=${personID} AND strasse="${
            args.strasse
          }" AND plz="${args.plz}" AND ort="${args.ort}"`
        );
        if (adressen.length === 0) {
          await query(
            `INSERT INTO adressen (personID, strasse, plz, ort) VALUES (${personID},"${
              args.strasse
            }","${args.plz}","${args.ort}")`
          );
        ...
        const vorhandeneAnmeldungen = await query(
          `SELECT anmeldeID FROM anmeldungen WHERE personID=${personID} AND veranstaltungsID=${
            args.veranstaltungsID
          }`
        );
        ...
        if (vorhandeneAnmeldungen.length > 0) {
          ...
          if (args.position === 1) {
            ...
            if (generateFlag) {
              ...
              await query(
                `INSERT INTO fzAntrag(personID) VALUES (${personID})`
              );
            }
          }
          await Promise.all([
  ...
  anmeldungBestaetigungsbrief: {
    ...
    resolve: handleAllowed(async function(_, args) {
      await query(
        `UPDATE anmeldungen SET bestaetigungsBrief=CURRENT_TIMESTAMP WHERE anmeldeID = '${
          args.anmeldeID
        }'`
      );
      return true;
  ...
  anmeldunginfobrief: {
    ...
    resolve: handleAllowed(async function(_, args) {
      await query(
        `UPDATE anmeldungen SET infoBrief=CURRENT_TIMESTAMP WHERE anmeldeID = '${
          args.anmeldeID
        }'`
      );
      return true;
